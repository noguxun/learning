...
#define NS_OUTPUT_PREFIX "[mynandsim]"
...
static int nandsim_debugfs_create(struct nandsim *dev)
{
	...
	dent = debugfs_create_dir("mynandsim", NULL);
	if (IS_ERR_OR_NULL(dent)) {
		int err = dent ? -ENODEV : PTR_ERR(dent);

		NS_ERR("cannot create \"mynandsim\" debugfs directory, err %d\n",
			err);
		return err;
	}
	dbg->dfs_root = dent;
...
static int alloc_device(struct nandsim *ns)
{
	...
	ns->nand_pages_slab = kmem_cache_create("mynandsim",
...
static int init_nandsim(struct mtd_info *mtd)
{
	...
	if (NS_IS_INITIALIZED(ns)) {
		NS_ERR("init_mynandsim: mynandsim is already initialized\n");
		return -EIO;
	}
	...
	if (ns->geom.pgsz == 512) {
		...
		NS_ERR("init_mynandsim: unknown page size %u\n", ns->geom.pgsz);
		return -EIO;
	}
	...
	if (!ns->buf.byte) {
		NS_ERR("init_mynandsim: unable to allocate %u bytes for the internal buffer\n",
			ns->geom.pgszoob);
		ret = -ENOMEM;
		goto error;
	}
